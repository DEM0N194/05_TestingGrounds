...
AFirstPersonCharacter::AFirstPersonCharacter()
{
	...
	// Note: The skeletal mesh Mesh1P is set in the derived blueprint asset named MyCharacter to avoid direct content references in C++.
}
...
void AFirstPersonCharacter::BeginPlay()
{
	...
	{
		if (GEngine) GEngine->AddOnScreenDebugMessage(-1, 10.0f, FColor::Red, TEXT("Error: Gun_BP in FirsPersonCharacter.uasset is set to NONE!"));
	}
	Gun = GetWorld()->SpawnActor<AGun>(GunBlueprint);
	Gun->AttachToComponent(Mesh1P, FAttachmentTransformRules(EAttachmentRule::SnapToTarget, true), FName("GripPoint"));
	Gun->SetAnimInstance(Mesh1P->GetAnimInstance(), nullptr);
	...
	// Note: we do it here because the Gun object is not yet set at the exectuion time of SetupPlayerInputComponent()
}
